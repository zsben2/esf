...
void solve_S1_1(mpq_t*);
mpq_t *get_T_when_k_is_1(const char*, const int);
...
int main() {
    ...
    mpq_t *T_1 = get_T_when_k_is_1("T.bin", 0);
...
void solve_S1_1(mpq_t *T_1) {
    ...
    */
    for (n = 1; n <= N; n++) {
        ...
        // 但是在并行文件调用计算的时候, 因为需要调用 S[n-1][i][1] 的数据,
        // 所以需要调用名为 n - 1 的文件
...
mpq_t *get_T_when_k_is_1(const char *file_name, const int is_read_T_file) {
    int n;
    mpq_t *T_1 = mpq_malloc_init_list(N + 1); // T_1[0] = 0

    if (is_read_T_file) {
        mpq_t **T = read_T(file_name);
        // n >= k = 1 有效
        for (n = 1; n <= N; n++) {
            mpq_set(T_1[n], T[n][1]);
        }
